...
1. Search for a regexp[fn:1]) from text in current buffer by
   external command[fn:2].
   
   [fn:1] You can write regexp with syntax of external command.
   
   [fn:2] External commands written in Perl (v5.8 or later
